...
const platformConfig = {
  bilibili: {
    searchUrl: "https://search.bilibili.com/bangumi?keyword=",
...
// 已移除hideBangumiList函数，因为我们希望动漫列表一直显示
// 已移除所有iframe相关代码
...
function renderBangumiList(bangumiData) {
  ...
  // 获取或创建bangumi-container元素
  let container = bangumiList.querySelector('.bangumi-container');
  if (!container) {
    container = document.createElement('div');
    container.className = 'bangumi-container';
    bangumiList.appendChild(container);
  } else {
    container.innerHTML = "";
  }
  ...
  bangumiData.forEach((item) => {
    const platform = extractPlatformFromInfobox(item.infobox) || '未知平台';
    const name = item.name.split(' ')[0] || item.name_cn || "未知动漫";
    
    // 创建标签元素
    const tag = document.createElement("span");
    tag.className = "bangumi-tag";
    tag.setAttribute('data-platform', platform);
    ...
    tag.addEventListener("click", () => {
      ...
      // 根据提取的平台设置下拉框选项
      if (platform !== '未知平台' && platformSelect) {
...
window.addEventListener("load", init);
   ...
   const DANMAKU_PROXY_URL = "https://fc.lyz05.cn/?url=";
// 更新拼接后的URL
