...
class SubtitleManager extends ChangeNotifier {
  ...
  // 保存视频与字幕路径的映射
  Future<void> saveVideoSubtitleMapping(String videoPath, String subtitlePath) async {
    try {
      ...
      debugPrint('SubtitleManager: 保存视频字幕映射 - 视频: $videoPath, 字幕: $subtitlePath');
    } catch (e) {
  ...
  Future<String?> getVideoSubtitlePath(String videoPath) async {
    try {
      ...
      debugPrint('SubtitleManager: 获取视频对应的字幕路径 - 视频: $videoPath, 字幕: $subtitlePath');
    ...
    try {
      ...
      if (externalSubtitlePath == null || externalSubtitlePath.isEmpty) {
        ...
        if (subtitleTrackInfo.containsKey('external_subtitle') && 
          ...
          externalSubtitlePath = subtitleTrackInfo['external_subtitle']?['path'] as String?;
        }
      }
      
      // 输出详细调试信息
      debugPrint('SubtitleManager: getCurrentSubtitleText - 外部字幕路径: $externalSubtitlePath');
      debugPrint('SubtitleManager: getCurrentSubtitleText - 激活轨道: ${_player!.activeSubtitleTracks}');
  ...
  Future<void> autoDetectAndLoadSubtitle(String videoPath) async {
    try {
      ...
      if (_player != null && _player!.activeSubtitleTracks.isNotEmpty) {
        // 排除轨道0（外部字幕轨道）
        hasActiveEmbeddedTrack = _player!.activeSubtitleTracks.any((track) => track > 0);
      }
      
      // 如果以前手动设置过外部字幕，则始终尝试加载
      bool wasManuallySet = false;
      if (_subtitleTrackInfo.containsKey('external_subtitle')) {
        wasManuallySet = _subtitleTrackInfo['external_subtitle']?['isManualSet'] == true;
      }
  ...
  void updateEmbeddedSubtitleTrack(int trackIndex) {
    ...
    final track = _player!.mediaInfo.subtitle![trackIndex];
  ...
  void updateAllSubtitleTracksInfo() {
    ...
    if (_player!.activeSubtitleTracks.isNotEmpty) {
      ...
      if (activeIndex > 0 && activeIndex <= _player!.mediaInfo.subtitle!.length) {
        // 激活的是内嵌字幕轨道
