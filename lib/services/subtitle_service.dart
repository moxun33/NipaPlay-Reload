...
class SubtitleService {
  ...
  /// 加载指定视频的外部字幕列表
  Future<List<Map<String, dynamic>>> loadExternalSubtitles(String videoPath) async {
    if (kIsWeb) return [];
    ...
    try {
      ...
      if (subtitlesJson != null) {
        ...
        final subtitles = decoded.map((item) => Map<String, dynamic>.from(item)).toList();
  ...
  /// 保存外部字幕列表
  Future<void> _saveExternalSubtitles(String videoPath, List<Map<String, dynamic>> subtitles) async {
    if (kIsWeb) return;
  ...
  Future<Map<String, dynamic>?> pickAndLoadSubtitleFile() async {
    ...
    try {
      ...
      // 检查文件格式
      final validExtensions = ['.srt', '.ass', '.ssa', '.sub'];
      final extension = p.extension(filePath).toLowerCase();
      
      if (!validExtensions.contains(extension)) {
        throw UnsupportedError('不支持的字幕格式，请选择 .srt, .ass, .ssa 或 .sub 文件');
      }
  ...
  /// 添加外部字幕
  Future<bool> addExternalSubtitle(String videoPath, Map<String, dynamic> subtitleInfo) async {
    try {
      final subtitles = await loadExternalSubtitles(videoPath);
      
      // 检查是否已经存在相同路径的字幕
      final existingIndex = subtitles.indexWhere((s) => s['path'] == subtitleInfo['path']);
      if (existingIndex >= 0) {
        // 已存在，更新信息
  ...
  /// 设置外部字幕激活状态
  Future<bool> setExternalSubtitleActive(String videoPath, int index, bool isActive) async {
    try {
      final subtitles = await loadExternalSubtitles(videoPath);
  ...
  String? findDefaultSubtitleFile(String videoPath) {
    ...
    try {
      ...
      // 常见字幕文件扩展名
      const subtitleExts = ['.srt', '.ass', '.ssa', '.sub'];
